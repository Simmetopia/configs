import { RunAgentResumeItem } from './types.js';
...
export type ItemInterruptErrorCode = 'invalid-payload' | 'invalid-edited-args' | 'invalid-tool-output' | 'invalid-response-schema' | 'unknown-interrupt' | 'expired' | 'stale' | 'conflict' | 'legacy-unsupported';
export type BatchInterruptErrorCode = 'incomplete-batch' | 'item-validation-failed' | 'unsupported-bulk-operation' | 'async-resolver' | 'inactive-transaction' | 'mixed-provenance' | 'transport' | 'server' | 'protocol' | 'invalid-response-schema' | 'expired' | 'stale' | 'conflict' | 'legacy-submit-failed';
 ...
 */
export declare const INTERRUPT_BINDING_VERSION: 1;
...
export type InterruptBinding = (ResponseSchemaInterruptBindingBase & {
    kind: 'tool-approval';
    toolName: string;
    toolCallId: string;
...
export type UnopenedInterruptBinding = InterruptBinding extends infer TBinding ? TBinding extends InterruptBinding ? Omit<TBinding, 'interruptedRunId' | 'generation'> : never : never;
export type ToolApprovalResolution = boolean | {
    approved: true;
    editedArgs?: unknown;
    payload?: unknown;
} | {
...
export declare function canonicalizeInterruptResolutions(resolutions: ReadonlyArray<RunAgentResumeItem>): {
