...
export interface ChatSandboxHooks<TContext = unknown> {
    onFile?: (ctx: ChatMiddlewareContext<TContext>, e: SandboxFileHookEvent) => void | Promise<void>;
    onFileCreate?: (ctx: ChatMiddlewareContext<TContext>, e: SandboxFileHookEvent) => void | Promise<void>;
    onFileChange?: (ctx: ChatMiddlewareContext<TContext>, e: SandboxFileHookEvent) => void | Promise<void>;
    onFileDelete?: (ctx: ChatMiddlewareContext<TContext>, e: SandboxFileHookEvent) => void | Promise<void>;
}
/**
 ...
 */
export type ChatMiddlewarePhase = 'init' | 'beforeModel' | 'afterModel' | 'modelStream' | 'beforeTools' | 'afterTools' | 'structuredOutput';
export declare const INTERRUPT_BOUNDARY_PHASES: readonly ["beforeModel", "afterModel", "beforeTools", "afterTools"];
export type InterruptBoundaryPhase = (typeof INTERRUPT_BOUNDARY_PHASES)[number];
export declare const INTERRUPT_TOOL_RESUMES: readonly ["continue", "cancel", "stop"];
...
type InterruptResponse<TDefinition> = TDefinition extends InterruptDefinition<any, any, infer TResponseSchema, any> ? TResponseSchema extends StandardSchemaV1<any, infer TResponse> ? TResponse : TResponseSchema extends StandardJSONSchemaV1<any, infer TResponse> ? TResponse : unknown : unknown;
export type GenericInterruptResolution<TDefinition extends AnyInterruptDefinition> = TDefinition extends AnyInterruptDefinition ? {
...
export interface InterruptResolutionCollection<TDefinitions extends AnyInterruptDefinition = AnyInterruptDefinition> {
    for: <TDefinition extends ([TDefinitions] extends [never] ? AnyInterruptDefinition : TDefinitions)>(definition: TDefinition) => ReadonlyArray<GenericInterruptResolution<TDefinition>>;
    all: {
        (): ReadonlyArray<GenericInterruptResolution<TDefinitions>>;
        <const TSelected extends ReadonlyArray<TDefinitions>>(...definitions: TSelected): ReadonlyArray<GenericInterruptResolution<TSelected[number]>>;
    };
}
type BivariantInterruptResolutionHook<TContext, TDefinitions extends AnyInterruptDefinition> = InterruptResolutionHookSignature<TContext, TDefinitions>['call'];
declare abstract class InterruptResolutionHookSignature<TContext, TDefinitions extends AnyInterruptDefinition> {
    abstract call(ctx: ChatMiddlewareContext<TContext>, resolutions: InterruptResolutionCollection<TDefinitions>): InterruptResolutionResult | Promise<InterruptResolutionResult>;
}
export type InterruptBoundaryResult<TDefinitions extends AnyInterruptDefinition = AnyInterruptDefinition> = undefined | {
...
export interface ChatResumeToolState {
    ...
    genericInterrupts?: ReadonlyMap<string, ChatResumeGenericResolution> | undefined;
    /** Durable generic requests reconstructed by server middleware. */
    genericInterruptRequests?: ReadonlyMap<string, GenericInterruptRequest<InterruptDefinition<any, any, any, any>>> | undefined;
...
export interface ChatMiddleware<TContext = unknown, TInterruptDefinitions extends AnyInterruptDefinition = never> {
     ...
     */
    onInterruptBoundary?: (ctx: ChatMiddlewareContext<TContext> & {
     ...
     */
    onInterruptResolution?: BivariantInterruptResolutionHook<TContext, TInterruptDefinitions>;
    /**
     ...
     */
    onShouldContinue?: (ctx: ChatMiddlewareContext<TContext>, state: AgentLoopState) => boolean | void | Promise<boolean | void>;
    /**
