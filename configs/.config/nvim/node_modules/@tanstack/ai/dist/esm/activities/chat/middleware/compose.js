...
//#region src/activities/chat/middleware/compose.ts
...
var MiddlewareRunner = class {
	...
	async runOnInterruptBoundary(ctx) {
		const requests = [];
		for (const mw of this.middlewares) if (mw.onInterruptBoundary) {
			...
			if (!skip) {
				this.logger.middleware(`hook=onInterruptBoundary middleware=${mw.name ?? "unnamed"}`, {
				...
				aiEventClient.emit("middleware:hook:executed", {
					...
					hasTransform: result?.interrupts !== void 0
				});
			}
		}
		return requests;
	}
	async runOnInterruptResolution(ctx, resolutions) {
		let toolResume;
		for (const mw of this.middlewares) if (mw.onInterruptResolution) {
			const skip = shouldSkipInstrumentation(mw);
			const start = Date.now();
			const next = await mw.onInterruptResolution(ctx, resolutions);
			if (next?.toolResume !== void 0) {
				const priority = {
					continue: 0,
					cancel: 1,
					stop: 2
				};
				if (toolResume === void 0 || priority[next.toolResume] > priority[toolResume]) toolResume = next.toolResume;
			}
			if (!skip) {
				this.logger.middleware(`hook=onInterruptResolution middleware=${mw.name ?? "unnamed"}`, {
					middleware: mw.name ?? "unnamed",
				...
				aiEventClient.emit("middleware:hook:executed", {
					...
					iteration: ctx.iteration,
					duration: Date.now() - start,
					hasTransform: next !== void 0
				});
			}
		}
		return toolResume === void 0 ? {} : { toolResume };
	}
	/**
	...
	*/
	async runSetup(ctx) {
		ctx.capabilities.setOnDuplicate((name) => {
			this.logger.warn(`capability "${name}" was provided more than once; last provider wins`, { capability: name });
		});
		for (const mw of this.middlewares) if (mw.setup) {
			const skip = shouldSkipInstrumentation(mw);
			const start = Date.now();
			await mw.setup(ctx);
			if (!skip) {
				this.logger.middleware(`hook=setup middleware=${mw.name ?? "unnamed"}`, {
					middleware: mw.name ?? "unnamed",
					hook: "setup"
				});
				aiEventClient.emit("middleware:hook:executed", {
		...
		for (const mw of this.middlewares) for (const handle of mw.provides ?? []) if (!ctx.capabilities.has(handle)) throw new Error(`Middleware "${mw.name ?? "unnamed"}" declares it provides "${handle.capabilityName}" but never called provide() in setup().`);
	}
	/**
	...
	*/
	async runSandboxFile(ctx, event) {
		const typed = {
			create: "onFileCreate",
			change: "onFileChange",
			delete: "onFileDelete"
		...
		for (const mw of this.middlewares) {
			...
			for (const fn of [hooks.onFile, hooks[typed]]) {
				...
				try {
					...
					this.logger.sandbox(`hook=${typed} middleware=${mw.name ?? "unnamed"} threw`, {
						middleware: mw.name ?? "unnamed",
						error
					});
				}
	...
	* {@link runOnError}.
	*/
	async captureTerminalHook(mw, hookName, invoke) {
		try {
			await invoke();
			return;
		} catch (error) {
	...
	async runOnFinish(ctx, info) {
		...
		for (const mw of this.middlewares) {
			...
			if (hook) {
				const skip = shouldSkipInstrumentation(mw);
				const start = Date.now();
				const failure = await this.captureTerminalHook(mw, "onFinish", () => hook.call(mw, ctx, info));
				if (failure !== void 0) {
		...
		if (firstFailure !== void 0) throw failures.length === 1 ? firstFailure.error : new AggregateError(failures.map((f) => f.error), `${failures.length} middleware onFinish hooks failed: ` + failures.map((f) => f.middleware).join(", "));
	}
	/**
	...
	async runOnAbort(ctx, info) {
		for (const mw of this.middlewares) {
			...
			if (hook) {
				const skip = shouldSkipInstrumentation(mw);
				const start = Date.now();
				if (await this.captureTerminalHook(mw, "onAbort", () => hook.call(mw, ctx, info)) === void 0 && !skip) {
	...
	async runOnError(ctx, info) {
		for (const mw of this.middlewares) {
			...
			if (hook) {
				const skip = shouldSkipInstrumentation(mw);
				const start = Date.now();
				if (await this.captureTerminalHook(mw, "onError", () => hook.call(mw, ctx, info)) === void 0 && !skip) {
	...
	*/
	async runOnShouldContinue(ctx, state) {
		for (const mw of this.middlewares) if (mw.onShouldContinue) {
			const skip = shouldSkipInstrumentation(mw);
			const start = Date.now();
			const result = await mw.onShouldContinue(ctx, state);
			if (!skip) {
				this.logger.middleware(`hook=onShouldContinue middleware=${mw.name ?? "unnamed"}`, {
					middleware: mw.name ?? "unnamed",
