...
import { SpecTokenUsage, TokenUsageLeftover } from './utilities/ag-ui-usage.js';
...
export type MessagePart<TData = unknown> = TextPart | ImagePart | AudioPart | VideoPart | DocumentPart | ToolCallPart | ToolResultPart | ThinkingPart | StructuredOutputPart<TData> | UIResourcePart;
/**
...
type RuntimeContextField<TContext> = IsUnknown<TContext> extends true ? {
    /**
...
export type ToolExecuteFunction<TInput extends SchemaInput | undefined = SchemaInput, TOutput extends SchemaInput | undefined = SchemaInput, TContext = unknown> = undefined extends TContext ? (args: InferSchemaType<TInput>, context?: ToolExecutionContext<TContext>) => Promise<InferSchemaType<TOutput>> | InferSchemaType<TOutput> : (args: InferSchemaType<TInput>, context: ToolExecutionContext<TContext>) => Promise<InferSchemaType<TOutput>> | InferSchemaType<TOutput>;
/**
 ...
 * @see https://standardschema.dev/json-schema
 */
export interface Tool<TInput extends SchemaInput | undefined = SchemaInput, TOutput extends SchemaInput | undefined = SchemaInput, TName extends string = string, TContext = unknown> {
    /**
 ...
 */
export interface TextOptions<TProviderOptionsSuperset extends Record<string, any> = Record<string, any>, TProviderOptionsForModel = TProviderOptionsSuperset, TContext = unknown> {
    model: string;
    messages: Array<ModelMessage>;
 ...
 * `metadata.tanstack`.
 */
export interface RunFinishedEvent extends Pick<AGUIRunFinishedEvent, 'threadId' | 'runId' | 'result' | 'outcome' | 'timestamp' | 'rawEvent'> {
 ...
 */
export interface RunErrorEvent extends Pick<AGUIRunErrorEvent, 'message' | 'code' | 'timestamp' | 'rawEvent'> {
 ...
 */
export interface ToolCallStartEvent extends Pick<AGUIToolCallStartEvent, 'toolCallId' | 'toolCallName' | 'parentMessageId' | 'timestamp' | 'rawEvent'> {
 ...
 */
export interface ToolCallEndEvent extends Pick<AGUIToolCallEndEvent, 'toolCallId' | 'timestamp' | 'rawEvent'> {
    type: 'TOOL_CALL_END';
 ...
 */
export interface CustomEvent extends Pick<AGUICustomEvent, 'name' | 'value' | 'timestamp' | 'rawEvent'> {
    type: 'CUSTOM';
    metadata?: Record<string, any>;
}
/**
 ...
 * ```
 */
export interface StructuredOutputCompleteEvent<T = unknown> extends CustomEvent {
 ...
 */
export type TaggedCustomEvent<T = unknown> = StructuredOutputStartEvent | StructuredOutputCompleteEvent<T> | ApprovalRequestedEvent | ToolInputAvailableEvent;
 ...
 */
export interface RerankOptions<TProviderOptions extends object = Record<string, unknown>> {
    model: string;
...
export type PersistedArtifactActivity = 'image' | 'audio' | 'tts' | 'video' | 'transcription';
 ...
 */
export type EmbeddingModelInputModalitiesByName = Record<string, ReadonlyArray<EmbeddingModality>>;
/**
 ...
 */
export type EmbeddingInputItem = string | TextPart | ImagePart | EmbeddingContentParts;
 ...
 */
export type EmbeddingInputItemFor<TModalities extends EmbeddingModality = EmbeddingModality> = string | TextPart | EmbeddingItemByModality[TModalities];
/**
